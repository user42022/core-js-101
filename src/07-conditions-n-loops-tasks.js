...
function getSumBetweenNumbers(n1, n2) {
  return Array(n2 - n1 + 1).fill(0).map((_, idx) => idx + n1).reduce((a, b) => a + b);
}
...
function findFirstSingleChar(str) {
  ...
  while (index < str.length) {
    if (str.split(str[index]).join('').length === str.length - 1) {
      return str[index];
    }
    index += 1;
  }
...
function isCreditCardNumber(ccn) {
  const ccnArray = `${ccn}`.split('').map((e) => +e);
  if ((ccnArray.length - 1) % 2) {
    ccnArray.forEach((e, i) => {
      if (((i + 1) % 2) && (i !== (ccnArray.length - 1))) {
        ccnArray[i] = e * 2 > 9 ? e * 2 - 9 : e * 2;
      }
    });
  } else {
    ccnArray.forEach((e, i) => {
  ...
  return !(ccnArray.reduce((a, b) => a + b) % 10);
}
...
function getDigitalRoot(num) {
  const resNum = `${num}`.split('').map((e) => +e).reduce((a, b) => a + b);
  return resNum > 9 ? getDigitalRoot(resNum) : resNum;
}
...
function isBracketsBalanced(str) {
  ...
  str.split('').forEach((e) => {
    if (bracketMap[stack[stack.length - 1]] === e) {
      stack.pop();
    } else {
      stack.push(e);
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  const zeroPath = pathes[0];
  let i = 0;
  // eslint-disable-next-line no-loop-func
  while (pathesCopy.every((path) => path[0] === zeroPath[i]) && i < zeroPath.length) {
    str += zeroPath[i];
    pathesCopy.forEach((path, idx) => {
      pathesCopy[idx] = path.slice(1);
    });
    i += 1;
  }
  ...
  const cuttedStr = str.split('/').slice(0, str.split('/').length - 1);
  if (cuttedStr) {
    return `${cuttedStr.join('/')}/`;
  }
  return '';
}
